...
**Your answer:** The datasaurus_dozen data frame has 1,846 rows and 3 columns. The variables are dataset, x, and y.
...
```{r star-plot, fig.fullwidth=TRUE, eval = TRUE, fig.alt="Scatterplot of y versus x for the star dataset, with points forming the outline of a star"}
...
- The correlation coefficient for the star dataset is: -0.063.

- Compared to the dino dataset (r = -0.06447185), the star dataset's correlation is: Very similar but it is slightly closer to zero.
...
```{r circle-plot, fig.fullwidth=TRUE, eval = TRUE, fig.alt="Scatterplot of y versus x for the circle dataset, with points forming the outline of a circle"}
# Your code here
...
- The correlation coefficient for the circle dataset is: -0.068.

- Compared to the dino dataset, the circle dataset's correlation is: Very similar but slightly more negative.

- What do you notice about the correlation coefficients for dino, star, and circle? They are nearly identical and show very weak negative linear relationships despite having very different shapes.
...
```{r all-viz, eval=TRUE, fig.fullwidth=TRUE, fig.alt="Grid of scatterplots, one per dataset in the Datasaurus Dozen, arranged in 3 columns and colored by dataset, with no legend."}
